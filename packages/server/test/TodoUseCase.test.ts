...
describe("TodoUseCase", () => {
  it.scoped("should be created", () =>
    Effect.gen(function*() {
      ...
      assert.strictEqual(todoId, 1)
    }).pipe(
      Effect.provide(TodoUseCase),
      Effect.provide(makeTest()),
      Effect.provide(makeTestLayer(TodoPortDriven)({ create: (_todo) => Effect.succeed(TodoId.make(1)) })),
  ...
  it.scoped("should be deleted", () =>
    Effect.gen(function*() {
      const todos = yield* TodoPortDriving
      const todoId = yield* todos.delete(TodoId.make(1)).pipe(
        withSystemActor
      )
      assert.strictEqual(todoId, 1)
    }).pipe(
  ...
  it.scoped.fails("should be deleted with TodoErrorNotFound", () =>
    Effect.gen(function*() {
      const todos = yield* TodoPortDriving
      yield* todos.delete(TodoId.make(1)).pipe(
        withSystemActor
      )
    }).pipe(
      Effect.provide(TodoUseCase),
  ...
  it.scoped("should be readById", () => {
    ...
    return Effect.gen(function*() {
      const todos = yield* TodoPortDriving
      const todo = yield* todos.readById(TodoId.make(1)).pipe(
        withSystemActor
      )
  ...
  it.scoped.fails("should be readById with TodoErrorNotFound", () =>
    Effect.gen(function*() {
      const todos = yield* TodoPortDriving
      yield* todos.readById(TodoId.make(1)).pipe(
        withSystemActor
      )
    }).pipe(
      Effect.provide(TodoUseCase),
  ...
  it.scoped("should be update", () =>
    Effect.gen(function*() {
      const todos = yield* TodoPortDriving
      const todoId = yield* todos.update(TodoId.make(1), {}).pipe(
        withSystemActor
      )
      assert.strictEqual(todoId, 1)
    }).pipe(
  ...
  it.scoped.fails("should be update with TodoErrorNotFound", () =>
    Effect.gen(function*() {
      const todos = yield* TodoPortDriving
      yield* todos.update(TodoId.make(1), {}).pipe(
        withSystemActor
      )
    }).pipe(
      Effect.provide(TodoUseCase),
