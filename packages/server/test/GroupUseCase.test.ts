...
describe("GroupUseCase", () => {
  it.scoped("should be created", () =>
    Effect.gen(function*() {
      ...
      Effect.provide(makeTestLayer(GroupPortDriven)({ create: (_group) => Effect.succeed(GroupId.make(1)) })),
  ...
  it.scoped("should be deleted", () =>
    Effect.gen(function*() {
      const groups = yield* GroupPortDriving
      const groupId = yield* groups.delete(GroupId.make(1)).pipe(
        withSystemActor
      )
      assert.strictEqual(groupId, 1)
    }).pipe(
  ...
  it.scoped.fails("should be deleted with GroupErrorNotFound", () =>
    Effect.gen(function*() {
      const groups = yield* GroupPortDriving
      yield* groups.delete(GroupId.make(1)).pipe(
        withSystemActor
      )
    }).pipe(
      Effect.provide(GroupUseCase),
  ...
  it.scoped("should be readById", () => {
    ...
    return Effect.gen(function*() {
      const groups = yield* GroupPortDriving
      const group = yield* groups.readById(GroupId.make(1)).pipe(
        withSystemActor
      )
  ...
  it.scoped.fails("should be readById with GroupErrorNotFound", () =>
    Effect.gen(function*() {
      const groups = yield* GroupPortDriving
      yield* groups.readById(GroupId.make(1)).pipe(
        withSystemActor
      )
    }).pipe(
      Effect.provide(GroupUseCase),
  ...
  it.scoped("should be update", () =>
    Effect.gen(function*() {
      const groups = yield* GroupPortDriving
      const groupId = yield* groups.update(GroupId.make(1), {}).pipe(
        withSystemActor
      )
      assert.strictEqual(groupId, 1)
    }).pipe(
  ...
  it.scoped.fails("should be update with GroupErrorNotFound", () =>
    Effect.gen(function*() {
      const groups = yield* GroupPortDriving
      yield* groups.update(GroupId.make(1), {}).pipe(
        withSystemActor
      )
    }).pipe(
      Effect.provide(GroupUseCase),
