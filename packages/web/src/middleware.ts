...
export async function middleware(request: NextRequest) {
  const isAuth = ["/forgot-password", "/sign-in", "/sign-up"].some((route) => request.nextUrl.pathname.includes(route))
  const isProtectedRoute = ["/admin", "/user"].some((route) => request.nextUrl.pathname.includes(route))
  ...
  const cspHeader = "base-uri 'self' http://127.0.0.1:3002; " +
        "child-src 'none'; " +
