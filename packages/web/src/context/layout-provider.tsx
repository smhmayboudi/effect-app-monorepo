...
export function LayoutProvider({ children }: LayoutProviderProps) {
  const [collapsible, _setCollapsible] = useState<Collapsible>(() => {
    return (getCookie(LAYOUT_COLLAPSIBLE_COOKIE_NAME) as Collapsible) || LAYOUT_COLLAPSIBLE_DEFAULT;
  });

  const [variant, _setVariant] = useState<Variant>(() => {
    return (getCookie(LAYOUT_VARIANT_COOKIE_NAME) as Variant) || LAYOUT_VARIANT_DEFAULT;
  });
